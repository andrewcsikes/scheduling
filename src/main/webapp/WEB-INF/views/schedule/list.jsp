...
<body>
	...
	<%
	java.util.ArrayList<String> fields = (java.util.ArrayList<String>)request.getAttribute("fields");
		...
		for(int x=0; x<26; x++){
			...
			for(String field:fields){		
				...
				if(hour == null && (user.getUserType().equals(com.vasa.scheduling.enums.UserType.ADMIN) ||
					...
					out.append("<td><a href='list/add?date="+format2.format(time.getTime())+"&field="+field+"'><img src='/scheduling/images/plus-icon.png' /></a></td>");
				}else if(hour != null && hour.equals(user.getTeam().getName() + " - " + user.getLastName() + " - " + user.getTeam().getAgeGroup().getName())){
					out.append("<td><a href='list/delete?date="+format2.format(time.getTime())+"&field="+field+"'><img src='/scheduling/images/minus-icon.png' /></a> "+hour+"</td>");
				}else{
